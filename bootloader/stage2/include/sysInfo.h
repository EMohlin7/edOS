...
/// @param input uint32 value that will be placed in eax and decide what info will be returned.
...
static inline void cpuId(uint32_t input, uint32_t* ebx, uint32_t* edx, uint32_t* ecx){
    __asm__("cpuid \n\t"                              \
            : "=b" (*ebx), "=d" (*edx), "=c" (*ecx)   \
            : "a" (input)
    );
